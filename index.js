...
async function getToken(v, email) {
  ...
  const response = await axios.post(
    "https://api.fortress-ww-prd.lightricks.com/v2/auth/otp?app=" +
      app +
      "&cvc=1412&plt=a&pltv=28&env=production",
    {
      identityType: "email",
      identity: email,
    },
    ...
    if (inbox != undefined) {
      try {
        ...
        const tokens = await axios.post(
          "https://api.fortress-ww-prd.lightricks.com/v2/auth/email/login?app=" +
            app +
            "&cvc=1412&plt=a&pltv=28&env=production",
          { email: email, identityProofType: "email", identityProof: kode },
          { headers: headers },
        );
  ...
  try {
    ...
    soket.ev.on("messages.upsert", async ({ messages }) => {
      try {
        ...
        //console.log(m)
        if (
          Object.keys(m2).includes("imageMessage") ||
          Object.keys(m2).includes("videoMessage")
        ) {
          ...
          if (mCaption.split(" ")[0] === ".toUrl") {
            ...
            if (type != "videoMessage") {
              const webp = await ToStic(media, {
                author: "Tamsis X Code",
                packname: "SimpleBot",
              });
              stickerMessage(fs.readFileSync(webp));
            } else {
              const webp = await writeExifVid(media, {
                author: "Tamsis X Code",
                packname: "SimpleBot",
              });
              stickerMessage(fs.readFileSync(webp));
            ...
            if (type != "videoMessage") {
              ...
              let result = (
                await axios.post(
                  "https://pyapi.cyclic.app/api/editor/vidio",
                  form,
                )
              ).data;
        ...
        if (typeof prompt === "string" && !m.eventResponses) {
          ...
          if (
            prompt.split(" ")[0] === ".toUrl" ||
            prompt.split(" ")[0] === ".sticker"
          ) {
          ...
          if (!member.hasOwnProperty(wa_number)) {
            if (prompt == "/daftar") {
              const gpts = (
                await axios.get(
                  "https://pyapi.cyclic.app/api/gpt3?prompt=balas%20pesan%20ini%20dengan%20kata%20sambutan%20untuk%20user%20yang%20baru%20terdaftar%20di%20layanan",
                )
              ).data;
              ...
              textMessage(
                pesan_belum_terdaftar[
                  Math.floor(Math.random() * pesan_belum_terdaftar.length)
                ],
              );
            }
